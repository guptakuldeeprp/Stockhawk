...
public class StockTaskService extends GcmTaskService {
    ...
    public int onRunTask(TaskParams params){
      ...
      if (urlStringBuilder != null){
        ...
        try{
          System.out.println("fetch Data URL: " + urlString);
  ...
  */
    ...
    public int onRunTask(TaskParams params) {
        ...
        if (params.getTag().equals("init") || params.getTag().equals("periodic")) {
            isUpdate = true;
            stockSymbols = Utils.fetchStockSymbols(this, true);
            if(stockSymbols == null || stockSymbols.isEmpty()) {
                stockSymbols.add("YHOO");
        ...
        if (!stockSymbols.isEmpty()) {
            ...
            try {
                Map<String, Stock> stocks = YahooFinance.get(stockSymbols.toArray(new String[0]));
                result = GcmNetworkManager.RESULT_SUCCESS;
                Utils.setStockStatus(mContext, STOCK_STATUS_OK);
                try {
                    ContentValues contentValues = new ContentValues();
                    // update ISCURRENT to 0 (false) so new data is current
                    if (isUpdate) {
                    ...
                    //Utils.setStockStatus(mContext, STOCK_STATUS_UNKNOWN);
                    Log.e(LOG_TAG, "Error applying batch insert", e);
                }
            } catch (IOException e) {
                ...
                Utils.setStockStatus(mContext, STOCK_STATUS_SERVER_INVALID);
                e.printStackTrace();
            }
        }
    ...
    private String getInvalidStockSymbol(Map<String, Stock> stocks) {
        for (String stockSymbol : stocks.keySet()) {
            if (stocks.get(stockSymbol).getName().equals(NA))
                return stockSymbol;
        }
